...
occurring at smaller scales. (Further information: [short video](https://www.youtube.com/watch?v=7xHsRkOdVwo))
...
the cluster are minimized. The figure below shows the *k*-means algorithm in action (*k*=2, [*source*](https://aman.ai/cs229/kmc/)). 
Training examples are shown as dots, and cluster centroids are shown as crosses.

<img src="Fig2.png" width="600" height="400">

(a) Original dataset. (b) Random initial cluster centroids (in this instance, not chosen to be equal to two training examples). (c-f) Illustration
of running two iterations of *k*-means (*k*=2). In each iteration, we assign each training example to the closest cluster centroid (shown by
“painting” the training examples the same color as the cluster centroid to which is assigned); then we move each cluster centroid to 
the mean of the points assigned to it. The algorithm converges when the assignments no longer change. (Further information: [short video](https://www.youtube.com/watch?v=4b5d3muPQmA))
...
The dataset with 4,345 genes (rows) and 43 experimental samples (columns) is used in the tutorial. It is a real study that investigated the expression of *E.coli*
...
How many genes are left after filtering? What does the parameter “0.50” in the 
...
Plot a heatmap and save the plot in .png file (instructions see above, don’t forget to change the file name):
```r
...
whether the determined cluster structure is stable. Therefore, we 
use the R package ‘pvclust’. Based on a bootstrapping approach, 
this method calculates a p-value for each cluster of the dendrogram. 
In this approach, many datasets are “generated” by repeated drawing 
from the existing data (the data are drawn by bootstrapping or 
“sampling with replacement”, and consequently, we can draw a dataset 
multiple times). If these many datasets generate the same clusters, 
then the clusters are “robust”. A significance value (p-value) here is 
provided at the end of the clustering procedure and can be between 0 
and 1 (0% – 100%). These p-values give an estimate of the cluster robustness.

![Fig.4](Fig4.png)

Please note, ‘pvclust()’ provides two types of p-values (in %): 
**au** (Approximately Unbiased) p-value and **bp** (Bootstrap 
Probability) value. Au p-value, which is computed by multiscale 
bootstrap resampling, is a better approximation to unbiased p-value 
...
```
Now check the clustering of the samples. Can you figure out some outliers? If so, try to remove these from your data set and repeat this section.
Hint: You can search a sample with the following command:
```r
grep("<..pattern..>", colnames(matrix.sel))
```

After identifying columns of outliers (e.g. 'number_col_1' and 'number_col_2'), we can remove them from the analysis:
```r
number_col_1 <- ...
number_col_2 <- ...
...
How many clusters are considered as stable (n_cl = number of stable clusters)? Answer:
```r
...
better for the practical course, select the clusters from ‘hclust()’
if ‘pvclust()’ was too slow on your computer.


Display the gene names of the chosen cluster (for ‘pv.clust()’, but with at least n = 100 bootraps):):
```r
cluster.sel <- 1
pv.pp[[1]][[cluster.sel]]
```
