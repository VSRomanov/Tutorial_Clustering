...
#heatmap.2(as.matrix(matrix.sel), Rowv=TRUE, Colv=as.dendrogram(clustering), key=TRUE, scale="row", 
#          col = redgreen(75), symkey=FALSE, density.info = "histogram", trace="none", labRow=rownames(as.matrix(matrix.sel[c(1:13),])))
dev.off()




## 6. Cluster stability (sample clusters)
...
## 7. Interpretation (functional analysis)
